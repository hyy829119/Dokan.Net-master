...
namespace Sshfs
{
    ...
    internal sealed class SftpFilesystem : SftpClient, IDokanOperations
    {
        ...
        public SftpFilesystem(ConnectionInfo connectionInfo, string rootpath, string label = null, bool useOfflineAttribute = false, bool debugMode = false, int attributeCacheTimeout = 5, int directoryCacheTimeout = 60)
            : base(connectionInfo)
        {
            _rootpath = rootpath;
        ...
        NtStatus IDokanOperations.CreateFile(string fileName, FileAccess access, FileShare share, FileMode mode, FileOptions options, FileAttributes attributes, IDokanFileInfo info)
        {
            ...
            if (info.IsDirectory)
            {
                ...
                if (attributesDir == null || attributesDir.IsDirectory || attributesDir.IsSymbolicLinkToDirectory)
                {
                    ...
                    {
                        DokanNet.NtStatus status = OpenDirectory(fileName, info);
                        try
                        {
                            if (status == DokanNet.NtStatus.ObjectNameNotFound)
                            {
                                GetAttributes(fileName);
                                return (DokanNet.NtStatus)0xC0000103L;
                            }
                        }
                        catch (SftpPathNotFoundException)
                        {
            ...
            if (fileName.EndsWith("desktop.ini", StringComparison.OrdinalIgnoreCase) || fileName.EndsWith("autorun.inf", StringComparison.OrdinalIgnoreCase)) //....
            {
                return DokanNet.NtStatus.NoSuchFile;
            }
            ...
            switch (mode)
            {
                case FileMode.Open:
                    if (sftpFileAttributes != null)
                    {
                        if (((uint)access & 0xe0000027) == 0 || sftpFileAttributes.IsDirectory)
                        {
                            ...
                            {
                                return DokanNet.NtStatus.Error;
                            }
                ...
                case FileMode.CreateNew:
                    if (sftpFileAttributes != null)
                        return DokanNet.NtStatus.ObjectNameCollision;
            ...
            try
            {
                info.Context = new SftpContext(this, path, mode, ((ulong)access & 0x40010006) == 0 ? System.IO.FileAccess.Read : System.IO.FileAccess.ReadWrite, sftpFileAttributes);
                if (sftpFileAttributes != null)
                {
            ...
            catch (SshException) // Don't have access rights or try to read broken symlink
            {
                ...
                if (sftpPathAttributes == null)
                {
                    ...
                    else
                    {
                        ...
                        return DokanNet.NtStatus.ObjectPathNotFound;
                    }
                }
        ...
        private DokanNet.NtStatus OpenDirectory(string fileName, IDokanFileInfo info)
        {
#if DEBUG
            string processName = Process.GetProcessById(info.ProcessId).ProcessName;
            ...
            if (sftpFileAttributes != null)
            {
                ...
                {
                    return (DokanNet.NtStatus)0xC0000103L; //STATUS_NOT_A_DIRECTORY
                }
            ...
            return DokanNet.NtStatus.ObjectNameNotFound;
        }

        private DokanNet.NtStatus CreateDirectory(string fileName, IDokanFileInfo info)
        {
        ...
        DokanNet.NtStatus IDokanOperations.ReadFile(string fileName, byte[] buffer, out int bytesRead, long offset, IDokanFileInfo info)
        {
        ...
        DokanNet.NtStatus IDokanOperations.WriteFile(string fileName, byte[] buffer, out int bytesWritten, long offset, IDokanFileInfo info)
        {
            bytesWritten = 0;
        ...
        DokanNet.NtStatus IDokanOperations.FlushFileBuffers(string fileName, IDokanFileInfo info)
        {
        ...
        DokanNet.NtStatus IDokanOperations.GetFileInformation(string fileName, out FileInformation fileInfo, IDokanFileInfo info)
        {
            ...
            if (sftpFileAttributes.IsDirectory || sftpFileAttributes.IsSymbolicLink)
            {
        ...
        DokanNet.NtStatus IDokanOperations.FindFilesWithPattern(string fileName, string searchPattern, out IList<FileInformation> files, IDokanFileInfo info)
        {
            files = null;
            return DokanNet.NtStatus.NotImplemented;
        }
        ...
        NtStatus IDokanOperations.FindFiles(string fileName, out IList<FileInformation> files, IDokanFileInfo info)
        {
            ...
            (files as List<FileInformation>).AddRange(sftpFiles.Select(file =>
                   {
                       ...
                       {
                           fileInformation.Attributes |= FileAttributes.NoScrubData | FileAttributes.System | FileAttributes.Device;
                       }
            ...
            {
                CacheAddAttr(GetUnixPath(String.Format("{0}\\{1}", fileName, file.Name)), file.Attributes, DateTimeOffset.UtcNow.AddSeconds(timeout));
            }

            try
            {
                CacheAddDir(GetUnixPath(fileName), new Tuple<DateTime, IList<FileInformation>>((info.Context as SftpContext).Attributes.LastWriteTime, files),
                           DateTimeOffset.UtcNow.AddSeconds(Math.Max(_attributeCacheTimeout, Math.Min(files.Count, _directoryCacheTimeout))));
            }
            catch
            {
            }
        ...
        DokanNet.NtStatus IDokanOperations.SetFileAttributes(string fileName, FileAttributes attributes, IDokanFileInfo info)
        {
        ...
        DokanNet.NtStatus IDokanOperations.SetFileTime(string fileName, DateTime? creationTime, DateTime? lastAccessTime, DateTime? lastWriteTime, IDokanFileInfo info)
        {
        ...
        DokanNet.NtStatus IDokanOperations.DeleteFile(string fileName, IDokanFileInfo info)
        {
            ...
            return
                UserCanWrite(
                    sftpFileAttributes)
                    ? DokanNet.NtStatus.Success
                    : DokanNet.NtStatus.AccessDenied;
        }

        DokanNet.NtStatus IDokanOperations.DeleteDirectory(string fileName, IDokanFileInfo info)
        {
            ...
            if (sftpFileAttributes == null)
            {
                ...
                {
                    return DokanNet.NtStatus.NoSuchFile;//not sure if can happen and what to return
                }
            }
            if (sftpFileAttributes.IsSymbolicLink)
            {
            ...
            if (dircache != null)
            {
                ...
                if (!test)
                {
                    ...
                    return DokanNet.NtStatus.DirectoryNotEmpty;
                }
                LogFSActionSuccess("DeleteDir", fileName, (SftpContext)info.Context, "");
                return DokanNet.NtStatus.Success;
            }
        ...
        DokanNet.NtStatus IDokanOperations.MoveFile(string oldName, string newName, bool replace, IDokanFileInfo info)
        {
        ...
        DokanNet.NtStatus IDokanOperations.SetEndOfFile(string fileName, long length, IDokanFileInfo info)
        {
            //Log("SetEnd");
        ...
        DokanNet.NtStatus IDokanOperations.SetAllocationSize(string fileName, long length, IDokanFileInfo info)
        {
            //Log("SetSize");
        ...
        DokanNet.NtStatus IDokanOperations.LockFile(string fileName, long offset, long length, IDokanFileInfo info)
        {
            LogFSActionError("LockFile", fileName, (SftpContext)info.Context, "NI");
            return DokanNet.NtStatus.Success;
        }

        DokanNet.NtStatus IDokanOperations.UnlockFile(string fileName, long offset, long length, IDokanFileInfo info)
        {
            LogFSActionError("UnlockFile", fileName, (SftpContext)info.Context, "NI");
            return DokanNet.NtStatus.Success;
        }

        DokanNet.NtStatus IDokanOperations.GetDiskFreeSpace(out long free, out long total, out long used, IDokanFileInfo info)
        {
            ...
            else
            {
                ...
                CacheAddDiskInfo(new Tuple<long, long, long>(free, total, used), DateTimeOffset.UtcNow.AddMinutes(3));
            }
            LogFSActionSuccess("GetDiskFreeSpace", this._volumeLabel, (SftpContext)info.Context, "Free:{0} Total:{1} Used:{2}", free, total, used);
            return DokanNet.NtStatus.Success;
        }

        DokanNet.NtStatus IDokanOperations.GetVolumeInformation(out string volumeLabel, out FileSystemFeatures features, out string filesystemName, out uint maximumComponentLength, IDokanFileInfo info)
        {
            ...
            maximumComponentLength = 20;
            features = FileSystemFeatures.CasePreservedNames | FileSystemFeatures.CaseSensitiveSearch | FileSystemFeatures.SupportsRemoteStorage | FileSystemFeatures.UnicodeOnDisk | FileSystemFeatures.SequentialWriteOnce;
        ...
        DokanNet.NtStatus IDokanOperations.GetFileSecurity(string filename, out FileSystemSecurity security, AccessControlSections sections, IDokanFileInfo info)
        {
        ...
        DokanNet.NtStatus IDokanOperations.SetFileSecurity(string filename, FileSystemSecurity security, AccessControlSections sections, IDokanFileInfo info)
        {
        ...
        DokanNet.NtStatus IDokanOperations.Unmounted(IDokanFileInfo info)
        {
            LogFSActionError("Unmounted", this._volumeLabel, (SftpContext)info.Context, "NI");
            return DokanNet.NtStatus.Success;
        }

        DokanNet.NtStatus IDokanOperations.Mounted(IDokanFileInfo info)
        {
            LogFSActionError("Mounted", this._volumeLabel, (SftpContext)info.Context, "NI");
            return DokanNet.NtStatus.Success;
        }

        DokanNet.NtStatus IDokanOperations.FindStreams(string fileName, out IList<FileInformation> streams, IDokanFileInfo info)
        {
