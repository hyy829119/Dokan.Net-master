    ...
    public partial class MainForm : Form
    {
        ...
        private void MountDrive(SftpDrive drive)
        {
            Task.Factory.StartNew(() =>
            {
                ...
                {
                    BeginInvoke(new MethodInvoker(() => { if ((drive.Tag as ListViewItem).Selected) { muButton.Enabled = true; } }));
                    if (Visible)
                    {
                        BeginInvoke(new MethodInvoker(() => MessageBox.Show(this, String.Format("{0} could not connect:\n{1}", drive.Name, e.Message), Text)));
                    }
                    else
                    {
